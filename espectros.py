...
HAC2 y HEC1.
Sus archivos Spectrum están en una carpeta en {base_path}/{individual}. Por ejemplo, un
Spectrum de una sílaba de HAC1 puede llamarse /home/user/datos_tesis/HAC1/spectrum1.txt
"""
...
def process_syllable_file(
    file_path,
):  # --> me da la lista de frecuencias de la sílaba input
...
# plt.tight_layout(h_pad=0.1)
for idx in range(4):
    axes[idx, 0].set_title(f"{individuals[idx]}", fontsize=16)
    scatter_alfa_1 = axes[idx, 0].scatter(
        df["Frequency"]
        .loc[df["Individual"] == f"{individuals[idx]}"]
        .loc[df["Tipo"] == "alfa"],
    ...
    axes[idx, 1].set_title(f"{individuals[idx+4]}", fontsize=16)
...
axes[3, 0].set_xticks(range(2000, 10000, 1000))
axes[3, 1].set_xticks(range(2000, 10000, 1000))
axes[3, 0].set_yticks(range(0, 2))
axes[3, 0].tick_params(axis="x", which="major", labelsize=16)
axes[3, 1].tick_params(axis="x", which="major", labelsize=16)
axes[0, 0].tick_params(axis="y", which="major", labelsize=16)
# Set common labels for all subplots
fig.text(0.5, 0.04, "frecuencia (Hz)", ha="center", va="center", fontsize=16)
fig.text(
    0.06,
    0.5,
    "dB/Hz relativizado al máximo de cada sílaba",
