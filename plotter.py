...
def plot_imgs_3x2(
    fila1, fila2, fila3, suptitle="", ax_titles=["solo de", "dueto", "HA"]
):
    ...
    for columna, foto in enumerate(fila1):
        if columna == 0:
            # ax_titles = ["HB", "HAC2", "HA"]
            x_range = (0, 2.7)
            axes[0, columna].set_xlim(x_range)
            axes[0, columna].set_xlabel("tiempo (s)", fontsize=9)
            xticks = [0, 1, 2, 3, 4]  # Example ticks

            # axes[0, columna].tick_params(axis="x", labelsize=20)  # Set xtick label size
            axes[0, columna].set_xticks(xticks)
            ...
            axes[0, columna].set_xlim(x_range)
            axes[0, columna].set_xlabel("tiempo (s)", fontsize=9)
            xticks = [0, 0.5, 1.0, 1.5]  # Example ticks
...
def plot_imgs_2x2(
    ...
    for columna, foto in enumerate(fila1):
        ...
        axes[0, columna].set_title("rango dinámico: 60dB")
...
def plot_superpuestos(lista_dirs_pitches, lista_pngs):
    ...
    for index, direc in enumerate(dirs_arriba):
        ...
        axes[0, index].set_xticks([])
        axes[0, index].set_yticks([])
        axes[0, index].set_title(indiv_name, fontsize=20)
        plt.show()
    ...
    for index, direc in enumerate(dirs_abajo):
        ...
        axes[2, index].set_title(indiv_name, fontsize=20)
        axes[3, index].set_xticks([])
        axes[3, index].set_yticks([])
        plt.show()
    axes[3, 0].set_xticks(np.linspace(0, 0.13, 3))
    axes[3, 0].set_ylabel("Frecuencia (Hz)", fontsize=16)
    axes[3, 0].set_yticks(np.linspace(850, 4500, 3))
    axes[3, 0].tick_params(labelsize=16)
    # axes[3, 0].set_ytickparams(fontsize=16)
    axes[3, 0].set_xlabel("Tiempo (s)", fontsize=16)
...
def comparar_red_kmeans(
    ...
    fig.suptitle(
        "Visualization of cluster quality formed by the embeddings of spectrograms on "
        "the latent space of one siamese neural network trained on pairs of\n"
        "spectrograms of 8 different rufous hornerosby way of comparison with "
        "the clusters formed by the spherical K-Means algorithm on the same data"
    )
    axs = [ax1, ax2, ax3]

    return centroides_k, centroides_red, fig, axs
...
def plotear_silhouettes(
    ...
    # {{{ acá me saco de encima los sils que son de predicts que for some reason son todos puntos en el mismo lugar
...
def plot_distintos_k(predicts, lista_k, spherical=True, solo_centroides=False):
    fig = plt.figure()
    ...
    for i, j in enumerate(lista_k):
        plot_k(
            ...
            ax=axs[i],
            k=lista_k[i],
            solo_centroides=solo_centroides,
        )
    fig.suptitle(
        f"Clusters obtenidos con KMeans para k=6,7,8 y 9 sobre un mismo set de predicciones"
    )
