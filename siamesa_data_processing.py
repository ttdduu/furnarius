...
"""
esta func toma un diccionario que tiene keys y values de este modo:
{'individuo':[lista de sílabas]}. Hay un diccionario para train y otro para valid o
test. Returnea un diccionario: {[tensor de sílaba 1, tensor de sílaba 2]
:[0 o 1 si son mismo individuo o no, 'individuo de la 1era sílaba',
'individuo de la 2da sílaba']}
"""
...
""" Con esta func genero los train, valid y test splits, y los guardo en una carpeta que
especifico como arg (sets_path). La llamo antes de cada entrenamiento dentro de
siamesa.py. De este modo, en siamesa.py genero y guardo los sets (la idea es
randomizarlos antes de cada entrenamiento), e inmediatamente después los importo.

Lo más importante que recibe esta función es la carpeta pitches (con el nombre de
argumento base_path), que son los archivos de texto con columnas de tiempo y frecuencia.
El resto de los args son básicamente las carpetas donde guardar los sets creados.

Tengo ejemplos de cómo llamar esta función en siamesa.py, pero explico qué es cada
argumento:

tr_specs tiene que ser una lista con esta estructura: ['nombre con el cual guardar el
training set' , training split (por ej 0.8) del total de datos]

val_specs está defaulteado a None porque algunos datasets los quise generar con y otros
sin un set de validación (por lo que dije en el informe: tengo pocos datos). De este
modo, al no incluir un val_specs[1], tengo solo un split de train en 0.8 : test en 0.2.

sets_path es la carpeta donde guardar los sets de train y valid-test.

la lista individuals sirve porque esos son los nombres de las carpetas donde están las
canciones de cada individuo; cada canción con sus sílabas, que son archivos txt, pasados
a tensores por load_images_as_tensors.
